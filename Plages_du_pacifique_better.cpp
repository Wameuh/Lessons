...
bool lien(Position i, Position j, const std::vector<std::vector<int>> &matrix)
{
    if (i == j) return false;
    if (   std::abs(i.first-j.first) < 2
        && std::abs(i.second-j.second) < 2
        && matrix[i.first][i.second] != 1
        && matrix[j.first][j.second] != 1)
            return true;
    return false;
}
...
void dijkstra(std::size_t L, std::size_t H, Position i, std::vector<std::vector<int>> &matrix)
{
    assert(i.first < L);
    assert(i.second < H);
    int max_distance = L*H+1;
    std::vector<std::vector<int>> cumul_distances(L, std::vector<int>(H, max_distance));
    std::vector<std::vector<Position>> from(L, std::vector<Position>(H, {-1,-1}));

    cumul_distances[i.first][i.second] = 0;

    std::priority_queue<Step> pq;
    pq.push(Step{0, {-1,-1}, i});
    ...
    while (!pq.empty())
    {
        ...
        from[top.to.first][top.to.second] = top.from;
        cumul_distances[top.to.first][top.to.second] = top.cumul_distance;

        for (int k=-1; k < 2 ; k++)
        {
            for (int l=-1 ; l < 2 ; l++)
            {
                Position neighbour = {top.to.first+k,top.to.second+l};
                if (neighbour.first >= L || neighbour.second >= H || neighbour.first < 0 || neighbour.second < 0)
                    continue;
                if (matrix[neighbour.first][neighbour.second] != 0)
                    continue;
                matrix[neighbour.first][neighbour.second] = 3;
                pq.push(Step{top.cumul_distance+1, top.to, neighbour});
            }
        }
    }
}
...
int main ()
{
    ...
    std::cin >> L >> H ;
    std::vector<std::vector<int>> carte(L, std::vector<int>(H));
    ...
    // 
    std::cerr << "on va lancer dijkstra !" << std::endl;
    dijkstra(L, H, {0,0}, carte);
    std::cerr << "on a passé dijkstra" << std::endl;
